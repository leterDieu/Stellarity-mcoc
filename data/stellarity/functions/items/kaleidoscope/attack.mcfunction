...
execute if score @s[tag=!stellarity.items.kaleidoscope.cooldown] stellarity.items.kaleidoscope.charge matches ..32 as @e[type=!#kohara:invalid_targets,nbt={HurtTime:10s},distance=..7.5,sort=nearest,limit=1] at @s anchored feet rotated 0 0 positioned ^ ^1.57 ^ run function stellarity:items/kaleidoscope/charges/0
