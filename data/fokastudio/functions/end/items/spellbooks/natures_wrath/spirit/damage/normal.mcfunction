execute as @e[type=marker,limit=1,sort=nearest,tag=foka.natures_wrath.spirit] at @s run function fokastudio:end/items/spellbooks/natures_wrath/spirit/damage/as_marker/normal

execute if score #upgr foka.misc matches 0 run scoreboard players set #damage foka.misc 50
execute if score #upgr foka.misc matches 1 run scoreboard players set #damage foka.misc 60
